...
function Navbar() {
  ...
  return (
    <div>
      <div className='fixed to-0 right-0 w-11/12 -z-10 tracking-y-[-80%]'>
        <Image src={assets.header_bg_color} alt='' className='w-full' />
      </div>

      <nav className={`w-full fixed px-5 lg:px-10 xl:px-[8%] py-4 
      flex items-center justify-between z-50 ${isScroll ? "bg-white bg-opacity-50 backdrop-blur-lg shadow-sm" : ""}`}>
        <a href='#top'>
          <Image src={assets.logo} alt='' className='w-50 cursor-pointer mr-14' />
        </a>

        <ul className='hidden md:flex items-center gap-12 lg:gap-12 rounded-full px-12 py-3 bg-white shadow-sm'>
